document.addEventListener('DOMContentLoaded', () => {
    ...
    // Toggle password visibility, code generated by Claude AI 
    const passwordToggles = document.querySelectorAll('.password-toggle');

    passwordToggles.forEach(toggle => {
        toggle.addEventListener('click', function () {
            // Find the associated input field (sibling of the toggle button)
            const validationSpan = this.previousElementSibling;
            const passwordInput = validationSpan.previousElementSibling;
...
function initImagePreviewers() {
    document.querySelectorAll('.image-previewer').forEach(previewer => {
        ...
        if (previewer.dataset.initialized === 'true') {
            return;
        }
        ...
        // Add click event to the previewer
        previewer.addEventListener('click', () => fileInput.click());
        ...
        if (imagePreview &&
            imagePreview.tagName.toLowerCase() === 'img' &&
            imagePreview.src &&
            !imagePreview.src.includes('data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP') &&
            imagePreview.src !== window.location.href &&
            !imagePreview.src.endsWith('/')) {
            previewer.classList.add('selected');
        } else {
            // Make sure to remove the selected class if there's no real image
            previewer.classList.remove('selected');
        }

        // Mark this previewer as initialized
        previewer.dataset.initialized = 'true';
    });
}
